...
class PoseServer:
    def __init__(
        ...
        inference_short_side: int = 0,   # try 256 for speed
    ):
        self.window_size = min(window_size, 10)
    ...
    def add_frame(self, sid: str, frame: Frame) -> None:
        if self.inference_busy[sid]:
            print(f"[skip] Inference busy, dropping frame {frame.frame_id}")
            return
        self.buffers[sid].clear()   # keep only the newest
    ...
    def maybe_run_inference(self, sid: str) -> Optional[Dict]:
        ...
        #below line is commented - try and finish computation for each frame before the next one is brought in
        #frames = list(self.buffers[sid])
        frame = self.buffers[sid][-1]
...
async def post_frame(
    ...
    # build camera matrix (not currently used by vggt forward, but kept)
    K = make_K(intrinsics)
